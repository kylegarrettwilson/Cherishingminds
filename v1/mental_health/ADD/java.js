...
var waittime=300;        /* time between chat refreshes (ms) */

intUpdate=window.setTimeout("read_cont();", waittime);
chatwindow.value = "loading...";

function read_cont()         { zeit = new Date(); ms = (zeit.getHours() * 24 * 60 * 1000) + (zeit.getMinutes() * 60 * 1000) + (zeit.getSeconds() * 1000) + zeit.getMilliseconds(); ajax_request2("/mental_health/ADD/chat.txt?x=" + ms); }
...
function write_msg(msg1,nick1)     { ajax_request("/mental_health/ADD/w.php?m=" + escape(msg1) + "&n=" + escape(nick1)); }
